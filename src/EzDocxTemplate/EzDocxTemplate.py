...
class EzDocxTemplate:
                ...
                for i in range(len(inline)):
                    if keywordBracket in inline[i].text:
                        text = inline[i].text.replace(
                            str(keywordBracket), str(data[keyword])
                        )
                        inline[i].text = text
                        repl = True
                if not repl:
    ...
    def ReplaceAll(data, element):
        ...
        if isinstance(element, DocumentObject) or isinstance(
            element, BlockItemContainer
        ):
    ...
    def copy_element_after_paraph(
        placeholder: Paragraph, element=None, elements=[], deletePlaceholder=True
    ):
        if element:
